...
VBA programme that iterates through a set of csv files in the 'Ralphie Reactor' folder, and summarise the data, and save in a Excel spreadsheet. When a user clicks the 'Import Data' button, csv files in the 'Ralphie Reactor' folder are imported, and then an Excel file will be open. After that, the user can select a column range that they want to summarise. The range must be selected without the header - for example, "$B$3:$B$6". The 'Reset' button will delete all the data in the sheet.
